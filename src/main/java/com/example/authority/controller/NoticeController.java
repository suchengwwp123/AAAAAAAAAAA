 ...
 * @return
 */
@Operation(summary = "查询所有Notice")
@GetMapping
public Result findAll(){
        return Result.success(noticeService.list(new LambdaQueryWrapper<Notice>().orderByDesc(Notice::getId)));
        }
/**
  * 获取单个
  * @param id
...
public void exportByIds(@PathVariable String[] ids, HttpServletResponse response) throws IOException {
        ...
        List<Notice> list = noticeService.listByIds(Arrays.asList(ids));
        // 这里注意 有同学反应使用swagger 会导致各种问题，请直接用浏览器或者用postman
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        // 这里URLEncoder.encode可以防止中文乱码 当然和easyexcel没有关系
        String fileName = URLEncoder.encode("notice导出数据", "UTF-8").replaceAll("\\+", "%20");
        response.setHeader("Content-disposition", "attachment;filename*=utf-8''" + fileName + ".xlsx");
        EasyExcel.write(response.getOutputStream(), Notice.class).sheet("sheel1").doWrite(list);
...
public Result writeExcel(MultipartFile file) throws IOException {
        EasyExcel.read(file.getInputStream(), Notice.class, new PageReadListener<Notice>(dataList -> {
        dataList.forEach(entity -> noticeService.save(entity.toBuilder().id(null).build()));
        })).sheet().doRead();
