...
import com.example.authority.service.BannerService;
import com.example.authority.entity.Banner;
 ...
 * @description: BannerController前端控制器
 * @author:dyy
 * @Version 3.0
 **/
@Tag(name = "轮播列表 前端控制器")
...
public Result deleteByIds(@PathVariable String[]ids){
        return Result.success(bannerService.removeByIds(Arrays.asList(ids)));
        }
...
public void exportByIds(@PathVariable String[] ids, HttpServletResponse response) throws IOException {
        ...
        List<Banner> list = bannerService.listByIds(Arrays.asList(ids));
        // 这里注意 有同学反应使用swagger 会导致各种问题，请直接用浏览器或者用postman
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        // 这里URLEncoder.encode可以防止中文乱码 当然和easyexcel没有关系
        String fileName = URLEncoder.encode("banner导出数据", "UTF-8").replaceAll("\\+", "%20");
        response.setHeader("Content-disposition", "attachment;filename*=utf-8''" + fileName + ".xlsx");
        EasyExcel.write(response.getOutputStream(), Banner.class).sheet("sheel1").doWrite(list);
...
public Result writeExcel(MultipartFile file) throws IOException {
        EasyExcel.read(file.getInputStream(), Banner.class, new PageReadListener<Banner>(dataList -> {
        dataList.forEach(entity -> bannerService.save(entity.toBuilder().id(null).build()));
        })).sheet().doRead();
