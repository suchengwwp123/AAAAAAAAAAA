...
public class RoleController {
    ...
    public Result save(@RequestBody Role role) {
        ...
        if (roleService.list(
                new LambdaQueryWrapper<Role>().eq(Role::getPerms, role.getPerms())
        ).size()>0){
            throw new RuntimeException("存在相同的权限标识，请更换！");
        }
//        先新增role数据
 ...
 */
    ...
    public Result update(@PathVariable Long id, @RequestBody Role role) {
        if (roleService.list(
                new LambdaQueryWrapper<Role>().eq(Role::getPerms, role.getPerms()).ne(Role::getId, id)
        ).size()>0){
            throw new RuntimeException("存在相同的权限标识，请更换！");
        }
...
//        log.warn(JSONUtil.toJsonStr(role.getPermissionIds()));
    ...
    public Result findPage(
        ...
        LambdaQueryWrapper<Role> lambdaQueryWrapper= new LambdaQueryWrapper<Role>().like(Role::getName, name).orderByDesc(Role::getId);
        if (statu!=null){
            lambdaQueryWrapper.eq(Role::getStatu,statu);
        }
        return Result.success(roleService.page(new Page<>(pageNum, pageSize), lambdaQueryWrapper));
    }
