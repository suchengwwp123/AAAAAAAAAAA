<script setup >
  ...
  const rules = reactive({
                  ...
                  datetime: [{required: true, message: '必选项不能为空', trigger: 'blur'}],

                  content: [{required: true, message: '必选项不能为空', trigger: 'blur'}],

                  role: [{required: true, message: '必选项不能为空', trigger: 'blur'}],

                  sessionId: [{required: true, message: '必选项不能为空', trigger: 'blur'}],
  ...
  const handleBatchDel = async () => {
    ...
    await request.delete(`/history/batch/${ids}`)
    ElMessage({
      showClose: true,
      message: '批量删除成功',
      type: 'success'
  ...
  const load = async () => {
    const res = await request.get('/history/page', {
      params: {
    ...
    await formEl.validate(async (valid, fields) => {
      if (valid) {
        await request({
          ...
          url: form.id ? `/history/${form.id}` : '/history',
          data: form
        })
        ElMessage({
          showClose: true,
  ...
  const beforeBatchUpload = async (file) => {
    let fd = new FormData()
    fd.append('file', file)
    await request.post('/history/batch/upload', fd, {
      headers: {
...
<template>
  ...
  >
    ...
    >
                    ...
                    <el-form-item label="聊天账户" prop="name">
                      <el-input v-model="form.name"/>
                    </el-form-item>

                    <el-form-item label="聊天时间" prop="datetime">
  ...
  <el-row class="page">
    ...
    <el-col class="page-operate--layout">
      <!--   新增   -->
      <div v-permission="'sys:history:add'">
        <el-button type="primary" :icon="Plus" @click="handleAdd" size="small" plain
        >新增
        </el-button>
      </div>
      <!--   批量导入   -->
      <div v-permission="'sys:history:batch:upload'">
      ...
      <!--   批量删除   -->
      <div v-permission="'sys:history:batch:delete'">
        <el-popconfirm
      ...
      <div v-permission="'sys:history:batch:export'">
        <el-button
            type="warning"
            :icon="Download"
            :disabled="disabled"
    ...
    >
      ...
      >
        <el-table-column type="selection" width="55"/>
                ...
                <el-table-column prop="name" label="聊天账户" align="center" show-overflow-tooltip/>
                <el-table-column prop="datetime" label="聊天时间" align="center" show-overflow-tooltip/>
                <el-table-column prop="content" label="聊天内容" align="center" show-overflow-tooltip/>
                <el-table-column prop="role" label="角色" align="center" show-overflow-tooltip/>
                <el-table-column prop="sessionId" label="会话Id" align="center" show-overflow-tooltip/>
        <el-table-column label="操作" align="center" width="240">
          <template #default="scope">
            <div style="display: flex; justify-content: center; gap: 10px">
              <div v-permission="'sys:history:update'">
                <el-button
                    type="primary"
                    :icon="Edit"
                    size="small"
              ...
              <div v-permission="'sys:history:delete'">
                <el-popconfirm
                    confirm-button-text="确定"
                    cancel-button-text="取消"
                    :icon="InfoFilled"
