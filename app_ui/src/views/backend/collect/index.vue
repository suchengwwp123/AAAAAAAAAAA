<script setup >
  ...
  const load = async () => {
    ...
    //   articleList
    const resarticle = await request.get(`/article`)
    articleList.value =resarticle.data
    //   userList
...
<template>
  ...
  >
    ...
    >
                      ...
                      <el-form-item label="收藏新闻" prop="articleId">
                      ...
                      <el-form-item label="收藏用户" prop="userId">
  ...
  <el-row class="page">
        ...
        <el-row>
          <el-col>
                        ...
                        <el-form-item label-width="5px">
                          <el-select v-model="articleId"  placeholder="请选择收藏新闻" @change="load">
                            <el-option
                                v-for="(item,index) in articleList"
                                :label="item.name"
                                :value="item.id"
                        ...
                        <el-form-item label-width="5px">
                          <el-select v-model="userId"  placeholder="请选择收藏用户" @change="load">
                            <el-option
                                v-for="(item,index) in userList"
                                :label="item.username"
                                :value="item.id"
    ...
    <el-col class="page-operate--layout">
      <!--   新增   -->
      <div v-permission="'sys:collect:add'">
        <el-button type="primary" :icon="Plus" @click="handleAdd" size="small" plain
        >新增
        </el-button>
      </div>
      <!--   批量导入   -->
      <div v-permission="'sys:collect:batch:upload'">
      ...
      <!--   批量删除   -->
      <div v-permission="'sys:collect:batch:delete'">
        <el-popconfirm
      ...
      <div v-permission="'sys:collect:batch:export'">
        <el-button
            type="warning"
            :icon="Download"
            :disabled="disabled"
    ...
    >
      ...
      >
        <el-table-column type="selection" width="55"/>
                ...
                <el-table-column prop="articleId" label="收藏新闻"  show-overflow-tooltip>
                  <template #default="scope">
                    <span v-if="scope.row.article">{{scope.row.article.name}}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="userId" label="收藏用户"  show-overflow-tooltip>
                  <template #default="scope">
                    <span v-if="scope.row.user">{{scope.row.user.username}}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="createTime" label="收藏时间"  show-overflow-tooltip/>
        <el-table-column label="操作" align="center" width="240">
          <template #default="scope">
            <div class="page-table-editout--layout">
              <div v-permission="'sys:collect:update'">
                <el-button
                    type="primary"
                    :icon="Edit"
                    size="small"
              ...
              <div v-permission="'sys:collect:delete'">
                <el-popconfirm
                    confirm-button-text="确定"
                    cancel-button-text="取消"
                    :icon="InfoFilled"
