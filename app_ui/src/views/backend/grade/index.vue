<script setup >
  ...
  const rules = reactive({
                  ...
                  gradeName: [{required: true, message: '必选项不能为空', trigger: 'blur'}],

                  gradeCode: [{required: true, message: '必选项不能为空', trigger: 'blur'}],

                  gradeTeacher: [{required: true, message: '必选项不能为空', trigger: 'blur'}],

                  studentCount: [{required: true, message: '必选项不能为空', trigger: 'blur'}],

                  status: [{required: true, message: '必选项不能为空', trigger: 'blur'}],
  ...
  const handleUpdate = async (id) => {
    ...
    const res = await request.get(`/grade/${id}`)
  ...
  const handleBatchDel = async () => {
    ...
    await request.delete(`/grade/batch/${ids}`)
    ElMessage({
      showClose: true,
      message: '批量删除成功',
      type: 'success'
  ...
  const load = async () => {
    const res = await request.get('/grade/page', {
      params: {
    ...
    await formEl.validate(async (valid, fields) => {
      if (valid) {
        await request({
          ...
          url: form.id ? `/grade/${form.id}` : '/grade',
          data: form
        })
        ElMessage({
          showClose: true,
  ...
  const beforeBatchUpload = async (file) => {
    let fd = new FormData()
    fd.append('file', file)
    await request.post('/grade/batch/upload', fd, {
      headers: {
...
<template>
  ...
  >
    ...
    >
                    ...
                    <el-form-item label="班级编码" prop="gradeCode">
                      <el-input v-model="form.gradeCode"/>
                    </el-form-item>

                    <el-form-item label="班主任" prop="gradeTeacher">
                      <el-input v-model="form.gradeTeacher"/>
                    </el-form-item>

                    <el-form-item label="学生人数" prop="studentCount">
                      <el-input v-model="form.studentCount"/>
                    </el-form-item>
  ...
  <el-row class="page">
        ...
        <el-row>
          <el-col>
                        ...
                        <el-form-item label-width="5px">
                          <el-input v-model="gradeName" placeholder="输入班级名称"/>
                        </el-form-item>
    ...
    <el-col class="page-operate--layout">
      <!--   新增   -->
      <div v-permission="'sys:grade:add'">
        <el-button type="primary" :icon="Plus" @click="handleAdd" size="small" plain
        >新增
        </el-button>
      </div>
      <!--   批量导入   -->
      <div v-permission="'sys:grade:batch:upload'">
      ...
      <!--   批量删除   -->
      <div v-permission="'sys:grade:batch:delete'">
        <el-popconfirm
      ...
      <div v-permission="'sys:grade:batch:export'">
        <el-button
            type="warning"
            :icon="Download"
            :disabled="disabled"
    ...
    >
      ...
      >
        <el-table-column type="selection" width="55"/>
                ...
                <el-table-column prop="gradeName" label="班级名称" align="center" show-overflow-tooltip/>
                <el-table-column prop="gradeCode" label="班级编码" align="center" show-overflow-tooltip/>
                <el-table-column prop="gradeTeacher" label="班主任" align="center" show-overflow-tooltip/>
                <el-table-column prop="studentCount" label="学生人数" align="center" show-overflow-tooltip/>
        ...
        <el-table-column label="操作" align="center" width="240">
          <template #default="scope">
            <div style="display: flex; justify-content: center; gap: 10px">
              <div v-permission="'sys:grade:update'">
                <el-button
                    type="primary"
                    :icon="Edit"
                    size="small"
              ...
              <div v-permission="'sys:grade:delete'">
                <el-popconfirm
                    confirm-button-text="确定"
                    cancel-button-text="取消"
                    :icon="InfoFilled"
