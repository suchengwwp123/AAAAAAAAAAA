<script setup >
  ...
  const rules = reactive({
                  ...
                  sex: [{required: true, message: '必选项不能为空', trigger: 'change'}],

                  gid: [{required: true, message: '必选项不能为空', trigger: 'change'}],

                      img: [{required: true, message: '必选项不能为空', trigger: 'change'}],
                      file: [{required: true, message: '必选项不能为空', trigger: 'change'}],
  })
  ...
  const handleBatchDel = async () => {
    ...
    await request.delete(`/student/batch/${ids}`)
    ElMessage({
      showClose: true,
      message: '批量删除成功',
      type: 'success'
  ...
  const load = async () => {
    ...
    //   sexDictList
    const ressexDict = await request.get(`/dict/detail/1`)
    sexDictList.value =ressexDict.data
    //   gradeList
    // const resgrade = await request.get(`/grade`)
    // gradeList.value =resgrade.data
  }
  // 加载页面初始化调用load方法
  load()
    ...
    await formEl.validate(async (valid, fields) => {
      if (valid) {
        await request({
          ...
          url: form.id ? `/student/${form.id}` : '/student',
          data: form
        })
        ElMessage({
          showClose: true,
  ...
  const beforeBatchUpload = async (file) => {
    let fd = new FormData()
    fd.append('file', file)
    await request.post('/student/batch/upload', fd, {
      headers: {
...
<template>
    ...
    >
                  ...
                  <el-form-item label="学生性别" prop="sex">
                    <el-select v-model="form.sex" >
                      <el-option
                          v-for="(item,index) in sexDictList"
                          :label="item.name"
                          :value="item.id"
                          :key="index.toString()"
                      ></el-option>
                  ...
                  <el-form-item label="学生档案" prop="file">
                    <el-upload action="" :before-upload="uploadfile" :show-file-list="true">
                      <el-button type="primary" size="small">点击上传</el-button>
                    </el-upload>
                  </el-form-item>
  ...
  <el-row class="page">
        ...
        <el-row>
          <el-col>
                        ...
                        <el-form-item label-width="5px">
                          <el-input v-model="name" placeholder="输入学生姓名"/>
                        </el-form-item>



                        <el-form-item label-width="5px">
                          <el-select v-model="sex"  placeholder="请选择学生性别" @change="load">
                            <el-option
                                v-for="(item,index) in sexDictList"
                                :label="item.name"
                                :value="item.id"
                        ...
                        <el-form-item label-width="5px">
                          <el-select v-model="gid"  placeholder="请选择学生班级" @change="load">
                            <el-option
                                v-for="(item,index) in gradeList"
                                :label="item.name"
                                :value="item.id"
    ...
    <el-col class="page-operate--layout">
      <!--   新增   -->
      <div v-permission="'sys:student:add'">
        <el-button type="primary" :icon="Plus" @click="handleAdd" size="small" plain
        >新增
        </el-button>
      </div>
      <!--   批量导入   -->
      <div v-permission="'sys:student:batch:upload'">
      ...
      <!--   批量删除   -->
      <div v-permission="'sys:student:batch:delete'">
        <el-popconfirm
      ...
      <div v-permission="'sys:student:batch:export'">
        <el-button
            type="warning"
            :icon="Download"
            :disabled="disabled"
    ...
    >
      ...
      >
        <el-table-column type="selection" width="55"/>
                ...
                <el-table-column prop="name" label="学生姓名" align="center" show-overflow-tooltip/>
                <el-table-column prop="sex" label="学生性别" align="center" show-overflow-tooltip>
                  <template #default="scope">
                    <span v-if="scope.row.sexDict">{{scope.row.sexDict.name}}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="gid" label="学生班级" align="center" show-overflow-tooltip>
                  <template #default="scope">
                    <span v-if="scope.row.grade">{{scope.row.grade.name}}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="img" label="学生照片" align="center">
                  <template #default="scope">
                    <el-image
                ...
                <el-table-column prop="file" label="学生档案" align="center">
                  <template #default="scope">
                    <el-button class="ml-2" type="success" size="small" @click="dowload(scope.row.file)">下载</el-button>
                  </template>
                </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="240">
          <template #default="scope">
            <div style="display: flex; justify-content: center; gap: 10px">
              <div v-permission="'sys:student:update'">
                <el-button
                    type="primary"
                    :icon="Edit"
                    size="small"
              ...
              <div v-permission="'sys:student:delete'">
                <el-popconfirm
                    confirm-button-text="确定"
                    cancel-button-text="取消"
                    :icon="InfoFilled"
